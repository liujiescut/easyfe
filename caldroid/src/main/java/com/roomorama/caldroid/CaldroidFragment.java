...
public class CaldroidFragment extends DialogFragment {
     ...
     * @return
     */
    public OnItemClickListener getDateItemClickListener(final CaldroidGridAdapter adapter) {
        OnItemClickListener dateItemClickListener = new OnItemClickListener() {
            @Override
                ...
                if (caldroidListener != null) {
                    if (!enableClickOnDisabledDates) {
                        if ((minDateTime != null && dateTime
                                ...
                                .indexOf(dateTime) != -1) || dateTime.getMonth() != getMonth()) {
                            return;
                        }
                    }
                    ...
                    dates.add(dateTime);
                    adapter.updateDatetimeList(dates.get(0).getMonth(), dates.get(0).getYear());
                    adapter.setSelectedDates(dates);
                    adapter.notifyDataSetChanged();

                    caldroidListener.onSelectDate(date, view, position);
                }
            }
        };
    ...
    private void setupDateGridPages(View view) {
        ...
        for (int i = 0; i < NUMBER_OF_PAGES; i++) {
            ...
            dateGridFragment.setOnItemClickListener(getDateItemClickListener(adapter));
            dateGridFragment
