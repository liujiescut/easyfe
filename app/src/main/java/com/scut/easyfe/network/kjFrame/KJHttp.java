...
public class KJHttp {
    ...
    public Request<byte[]> form(int method , String url, HttpParams params, HttpCallBack callback) {
        if(null == params){
            params = new HttpParams();
        }
        //params.putQueryParams("", "");
        //params.put("", ""); //不知道为何，反正没有参数的时候就要加上一对空的参数，不然会报数组越界异常
        Request<byte[]> request = new FormRequest(method, url, params,
                callback);
    ...
    public Request<byte[]> json(int method , String url, HttpParams params,
                                    HttpCallBack callback) {
        Request<byte[]> request = new JsonRequest(method, url, params,
                callback);
        request.setShouldCache(false);
        doRequest(request);
        return request;
